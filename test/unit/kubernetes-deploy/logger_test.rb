...
class FormattedLoggerTest < Krane::TestCase
  def test_build
    new_logger = Krane::FormattedLogger.build('test-ns', 'minikube', @logger_stream)
  ...
  def test_verbose_tag_mode
    new_logger = Krane::FormattedLogger.build('test-ns', 'minikube', @logger_stream, verbose_prefix: true)
