...
class PodTest < KubernetesDeploy::TestCase
  ...
  def test_deploy_failed_is_true_for_container_config_error_post_18
    ...
    expected_msg = <<~STRING
      The following containers encountered errors:
      > hello-cloud: Failed to generate container configuration: Generate Container Config Failed
    STRING
    assert_equal expected_msg, pod.failure_message
  end
  ...
  def test_deploy_failed_is_true_for_generic_run_error
    container_state = {
      "lastState" => {
        "terminated" => {
          "reason" => "Error",
    ...
    expected_msg = <<~STRING
      The following containers encountered errors:
      > hello-cloud: Crashing (exit 1). See logs for more information.
    STRING
    assert_equal expected_msg, pod.failure_message
  end
