...
class ResourceCacheTest < Krane::TestCase
  ...
  def test_get_instance_populates_the_cache_and_returns_instance_hash
    ...
    stub_group_kind_get("FakePod.", items: pods.map(&:kubectl_response), times: 1)
  ...
  def test_get_instance_populates_the_cache_for_global_resources
    ...
    stub_group_kind_get("FakeNode.", items: nodes.map(&:kubectl_response), times: 1, use_namespace: false)
  ...
  def test_get_all_populates_cache_and_returns_array_of_instance_hashes
    ...
    stub_group_kind_get("Configmap.", items: configmaps.map(&:kubectl_response), times: 1)
  ...
  def test_if_kubectl_error_then_empty_result_returned_but_not_cached
    ...
    assert_equal([], @cache.get_all('FakeConfigMap.', "fake" => "false"))
  ...
  def test_get_all_with_selector_populates_full_cache_and_filters_results_returned
    ...
    stub_group_kind_get('FakeConfigMap.', items: all_cm.map(&:kubectl_response), times: 1)
  ...
  def test_concurrently_syncing_huge_numbers_of_resources_makes_exactly_one_kubectl_call_per_kind
    ...
    stub_group_kind_get("FakeDeployment.", items: deployments.map(&:kubectl_response), times: 1)
    stub_group_kind_get("FakePod.", items: pods.map(&:kubectl_response), times: 1)
    stub_group_kind_get("FakeConfigMap.", items: pods.map(&:kubectl_response), times: 1)
  ...
  def test_warming_the_resource_cache_works_for_custom_resources
    ...
    crd = ::Krane::ApiextensionsK8sIo::CustomResourceDefinition.new(namespace: 'test', context: 'nope',
    ...
    stub_group_kind_get("UnitTest.", times: 1)
    stub_group_kind_get("CustomResourceDefinition.apiextensions.k8s.io", times: 1)
    resources = [cr, crd]
  ...
  class MockResource
    ...
    def sync(mediator)
      mediator.get_all(group_kind)
      mediator.get_instance(group_kind, @name)
      @synced = true
    end
