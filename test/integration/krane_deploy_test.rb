...
class KraneDeployTest < Krane::IntegrationTest
  def test_full_hello_cloud_set_deploy_succeeds
    ...
    assert_logs_match_all([
      %r{ReplicaSet.apps/bare-replica-set\s+1 replica, 1 availableReplica, 1 readyReplica},
      %r{Deployment.apps/web\s+1 replica, 1 updatedReplica, 1 availableReplica},
      %r{Service./web\s+Selects at least 1 pod},
      %r{DaemonSet.apps/ds-app\s+#{num_ds} updatedNumberScheduled, #{num_ds} desiredNumberScheduled, #{num_ds} numberReady},
      %r{StatefulSet.apps/stateful-busybox},
      %r{Service./redis-external\s+Doesn't require any endpoint},
      "- Job.batch/hello-job (timeout: 600s)",
      %r{Job.batch/hello-job\s+(Succeeded|Started)},
    ])
  ...
  def test_role_and_role_binding_predeployed_before_unmanaged_pod
    ...
    hello_cloud.assert_unmanaged_pod_statuses("Succeeded", 1)
    assert_logs_match_all([
      %r{Successfully deployed in \d.\ds: Role.rbac.authorization.k8s.io/role},
      %r{Successfully deployed in \d.\ds: RoleBinding.rbac.authorization.k8s.io/role-binding},
...
unknown field \"myKey\" in io.k8s.apimachinery.pkg.apis.meta.v1.ObjectMeta",
  ...
  def test_crashing_container_on_deployment_fails_quickly
    ...
    assert_logs_match_all([
      "Failed to deploy 1 resource",
      "Deployment.apps/crash-loop: FAILED",
      "The following containers are in a state that is unlikely to be recoverable:",
  ...
  def test_unrunnable_container_on_deployment_pod_fails_quickly
    ...
    assert_logs_match_all([
      "Failed to deploy 1 resource",
      "Deployment.apps/cannot-run: FAILED",
      "The following containers are in a state that is unlikely to be recoverable:",
      "Crashing repeatedly",
  ...
  def test_deploy_result_logging_for_mixed_result_deploy
    ...
    assert_logs_match_all([
      %r{Deployment.apps/missing-volumes: TIMED OUT \(progress deadline: \d+s\)},
      "Timeout reason: ProgressDeadlineExceeded",
  ...
  def test_scale_existing_deployment_down_to_zero
    ...
    assert_logs_match_all([
      %r{Deployment.apps/web\s+1 replica, 1 updatedReplica, 1 availableReplica},
      %r{Service./web\s+Selects at least 1 pod},
      %r{Deployment.apps/web\s+0 replicas},
  ...
  def test_can_deploy_statefulset_with_zero_replicas
    ...
    assert_logs_match_all([
      ...
      %r{StatefulSet.apps/stateful-busybox\s+0 replicas},
    ])
  end
  ...
  def test_deploy_successful_with_multiple_template_paths
    ...
    assert_logs_match_all([
      ...
      %r{CronJob.batch/my-cronjob\s+Exists},
      %r{Deployment.apps/web\s+1 replica, 1 updatedReplica, 1 availableReplica},
  ...
  def test_bad_container_on_daemon_sets_fails
    ...
    assert_logs_match_all([
      "Failed to deploy 1 resource",
      "DaemonSet.apps/crash-loop: FAILED",
      "crash-loop-back-off: Crashing repeatedly (exit 1). See logs for more information.",
  ...
  def test_bad_container_on_stateful_sets_fails_with_rolling_update
    ...
    assert_logs_match_all([
      ...
      "StatefulSet.apps/stateful-busybox: FAILED",
  ...
  def test_rolling_update_stateful_sets_are_monitored
    ...
    assert_logs_match_all([
      ...
      "Successful resources",
      %r{StatefulSet.apps/stateful-busybox\s+2 replicas, 2 readyReplicas, 2 currentReplicas},
    ], in_order: true)
  end
  ...
  def test_jobs_can_fail
    ...
    assert_logs_match_all([
      "Deploying Job.batch/hello-job (timeout: 600s)",
      "Result: FAILURE",
      "Job.batch/hello-job: FAILED",
      "Final status: Failed",
      %r{\[Job.batch/hello-job\]\tDeadlineExceeded: Job was active longer than specified deadline \(\d+ events\)},
    ])
  end
  ...
  def test_resource_watcher_reports_failed_after_timeout
    ...
    assert_logs_match_all([
      ...
      "Deployment.apps/cannot-run: FAILED",
      bad_probe_timeout,
      "Deployment.apps/missing-volumes: GLOBAL WATCH TIMEOUT (20 seconds)",
    ])
  end
  ...
  def test_resource_watcher_raises_after_timeout_seconds
    ...
    assert_logs_match_all([
      ...
      "Deployment.apps/undying: GLOBAL WATCH TIMEOUT (5 seconds)",
  ...
  def test_network_policies_are_deployed_first
    ...
    assert_logs_match_all([
      "Predeploying priority resources",
      "Deploying NetworkPolicy.networking.k8s.io/allow-all-network-policy (timeout: 30s)",
      "Successfully deployed 1 resource",
      "Successful resources",
      "NetworkPolicy.networking.k8s.io/allow-all-network-policy",
    ], in_order: true)
  end
  ...
  def test_deploy_resource_using_generate_name_succeeds
    ...
    assert_logs_match_all([
      ...
      "PodDisruptionBudget.policy/test- (timeout: 10s)",
      %r{Pod./#{pod_name_matcher}\s+Succeeded},
      %r{PodDisruptionBudget.policy/#{pdb_name_matcher}\s+Available},
  ...
  def test_deploy_k8s_compatibility
    ...
    assert_logs_match_all([
      ...
      "Deploying Deployment.apps/web (progress deadline: 60s)", # annotation timeout override
      "Result: SUCCESS",
      "Successfully deployed 1 resource",
    ], in_order: true)
