...
class GlobalDeployTest < Krane::IntegrationTest
  def test_global_deploy_task_success
    ...
    assert_logs_match_all([
      ...
      "  - StorageClass.storage.k8s.io/#{storage_class_name}",
      "Phase 2: Checking initial resource statuses",
      %r{StorageClass.storage.k8s.io\/#{storage_class_name}\s+Not Found},
      "Phase 3: Deploying all resources",
      "Deploying resources:",
      %r{StorageClass.storage.k8s.io\/#{storage_class_name} \(timeout: 300s\)},
      %r{PriorityClass.scheduling.k8s.io/#{priority_class_name} \(timeout: 300s\)},
      "Don't know how to monitor resources of type StorageClass.storage.k8s.io.",
      %r{Assuming StorageClass.storage.k8s.io\/#{storage_class_name} deployed successfully.},
      %r{Successfully deployed in [\d.]+s: PriorityClass.scheduling.k8s.io/#{priority_class_name}, StorageClass.storage.k8s.io\/#{storage_class_name}},
      "Result: SUCCESS",
      "Successfully deployed 2 resources",
      "Successful resources",
      "StorageClass.storage.k8s.io/#{storage_class_name}",
      "PriorityClass.scheduling.k8s.io/#{priority_class_name}",
    ])
  end
  ...
  def test_global_deploy_task_success_timeout
    ...
    assert_logs_match_all([
      ...
      %r{StorageClass.storage.k8s.io\/#{storage_class_name}: GLOBAL WATCH TIMEOUT \(0 seconds\)},
  ...
  def test_global_deploy_task_success_verify_false
    ...
    assert_logs_match_all([
      ...
      "Phase 2: Checking initial resource statuses",
      %r{StorageClass.storage.k8s.io\/#{storage_class_name}\s+Not Found},
      %r{PriorityClass.scheduling.k8s.io/#{priority_class_name}\s+Not Found},
      "Phase 3: Deploying all resources",
      "Deploying resources:",
      %r{StorageClass.storage.k8s.io\/#{storage_class_name} \(timeout: 300s\)},
      %r{PriorityClass.scheduling.k8s.io/#{priority_class_name} \(timeout: 300s\)},
  ...
  def test_global_deploy_task_success_selector
    ...
    assert_logs_match_all([
      ...
      %r{StorageClass.storage.k8s.io\/#{storage_class_name} \(timeout: 300s\)},
      "Don't know how to monitor resources of type StorageClass.storage.k8s.io.",
      "Assuming StorageClass.storage.k8s.io/#{storage_class_name} deployed successfully.",
  ...
  def test_global_deploy_prune_success
    ...
    assert_logs_match_all([
      ...
      "  - StorageClass.storage.k8s.io/#{storage_class_name}",
      "Phase 2: Checking initial resource statuses",
      %r{StorageClass.storage.k8s.io\/#{storage_class_name}\s+Exists},
      "Phase 3: Deploying all resources",
      %r{Deploying StorageClass.storage.k8s.io\/#{storage_class_name} \(timeout: 300s\)},
      "The following resources were pruned: priorityclass.scheduling.k8s.io/#{priority_class_name}",
      %r{Successfully deployed in [\d.]+s: StorageClass.storage.k8s.io\/#{storage_class_name}},
      "Result: SUCCESS",
