  ...
  class RolloutConditions
    class << self
      def parse_config(config_string)
        ...
        config[:success_conditions].each do |query|
          query[:path] = JsonPath.new(query[:path]) if query.key?(:path)
        end
        config[:failure_conditions].each do |query|
          query[:path] = JsonPath.new(query[:path]) if query.key?(:path)
        ...
        raise RolloutConditionsError, "Error parsing rollout config: #{e}"
      ...
      def validate_config(config)
        ...
        missing = top_level_keys.reject { |k| config.key?(k) }
    ...
    def initialize(config)
      @success_conditions = config[:success_conditions]
