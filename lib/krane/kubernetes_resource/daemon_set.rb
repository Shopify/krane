  ...
  class DaemonSet < PodSetBase
    ...
    def rollout_annotation_err_msg
      "'#{Annotation.for(REQUIRED_ROLLOUT_ANNOTATION)}: #{required_rollout}' is invalid. " \
        "Acceptable values: #{REQUIRED_ROLLOUT_TYPES.join(', ')}, or a percentage (e.g. 90%)"
    end
    ...
    def validate_definition(*, **)
      ...
      update_strategy = @definition.dig('spec', 'updateStrategy', 'type').to_s
      if required_rollout.downcase == 'maxunavailable' && update_strategy.present? && update_strategy.downcase != 'rollingupdate'
        @validation_errors << "'#{Annotation.for(REQUIRED_ROLLOUT_ANNOTATION)}: #{required_rollout}' " \
          "is incompatible with updateStrategy '#{update_strategy}'"
      end
